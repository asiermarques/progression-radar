...
import {
  ...
  tags,
  compareToContainer,
  personLevelsContainer,
  compareToLevelsContainer
} from "./reducers/all";
...
import {
  NO_ROLES_EXCEPTION,
  ExampleService
} from "./domain/service/ExampleService";
...
try {
  if (!window.location.search) throw NO_INPUT_STATE;

  const state = processQueryString(
  ...
  const store = createStore(
    combineReducers({
      ...
      compareToLevels: compareToLevelsContainer(
        categoryService,
        roleRepositoryInstance
      )
    }),
